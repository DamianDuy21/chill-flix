...
const bannerIndexPageControlHandler = () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    const correspondingSliderItem = document.querySelector(
      `[slideritem-id="${sliderItemId}"]`
    );
    if (correspondingSliderItem) {
      correspondingSliderItem.classList.add("active");
    } else {
      console.error(
        `Corresponding slider item with ID ${sliderItemId} not found.`
      );
    }
  });
};
...
const bannerIndexPage = async () => {
  ...
  data = data.filter((item) => {
    return item != undefined && item.status == true;
  });
  console.log(data);
                ...
                ${data.map((item, index) => {
                  {
                    return `
                        <div class= "slider-item ${
                          index == 0 ? "active" : ""
                        }" slider-item slideritem-id=${index}>
                             <img src=${item.movie.thumb_url} alt=${
                      item.movie.name
                    } class="img-cover"
                                 loading="lazy">
                             <div class="banner-content">
                                 ...
                                 <a href="./watchMovie.html?${
                                   item.episodes[0].server_data.length > 1
                                     ? `${item.movie.slug}&tap-01`
                                     : `${item.movie.slug}&full`
                                 }" class="btn" watch-now-btn movie-slug=${
                      item.movie.slug
                    } movie-alike=${item.movie.category[0].slug} movie-name="${
                      item.movie.name
                    }">
                                 ...
                                 <a href="./detail.html?${
                                   item.movie.slug
                                 }" class="btn normal" movie-name="${
                      item.movie.name
                    }" movie-slug=${item.movie.slug} movie-alike=${
                      item.movie.category[0].slug
                    } todetail>
